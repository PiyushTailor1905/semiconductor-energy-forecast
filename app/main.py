...
try:
    ...
    xgb_model = None
    prophet_model = None
    feature_cols = []# main.py — FastAPI backend
# Serves semiconductor revenue forecasts via REST API
...
# ─────────────────────────────────────────────────────────────
# LOAD MODELS (Auto-resolves paths for Render)
# ─────────────────────────────────────────────────────────────
...
@app.get("/")
def root():
    return {"message": "EnergiChip Forecast API is running", "docs": "/docs"}

@app.get("/health")
def health_check():
    return {"status": "healthy", "model": "Prophet Forecast Model"}
...
def predict_revenue(request: ForecastRequest):
    try:
        ...
        if prophet_model is None:
            raise HTTPException(
                status_code=500,
                detail="Prophet model not loaded. Check model paths."
            )
        ...
        lower_bounds = future_forecast["yhat_lower"].round(2).tolist()
        upper_bounds = future_forecast["yhat_upper"].round(2).tolist()

        quarters = [f"{d.year}-Q{d.quarter}" for d in future_forecast["ds"]]
